...
function gateKeeper(req, res, next) {
  const userData = queryString.parse(req.get('x-username-and-password'));
  let user;
  if (userData.pass && userData.user){
    user = USERS.find(user => {
      return !user.password.localeCompare(userData.pass) && !user.userName.localeCompare(userData.user)
    })
  }
